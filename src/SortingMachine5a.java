...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (last - top > 0) {
            ...
            // Storing the position of the left sub tree as the smallest by default
            int smallest = left;
            // Checking whether the position of the left sub tree
            // is lesser than the last element
            if (left < last) {
                // Checking which of the two  sub trees is smaller
                if (order.compare(array.entry(left), array.entry(right)) > 0) {
                    // storing the smallest of the two sub trees
                    smallest = right;
                }
            }

            // Checking if siftDown operation can happen
            if (smallest <= last) {
                // Checking if the root is smaller than the smallest element
                if (order.compare(array.entry(top),
                        array.entry(smallest)) > 0) {

                    // Changing the entries so that the root can be removed and
                    //the tree can still be balanced and represented as a heap
                    array.exchangeEntries(top, smallest);

                    // Recursion calling siftDown
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        // Check whether right is last
        if (right < array.length()) {
            //recursion
            heapify(array, left, order);
            heapify(array, right, order);
        }
        // Other case when left might be last
        else if (left < array.length()) {
            // recursion
            heapify(array, left, order);
        }
        // Recursion above made sure everything reaches requires clause for
        // siftDown
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // Loop to make a binary tree from q
        int length = q.length();
    ...
    public final T removeFirst() {
        ...
        // Make sure everything follows order.
